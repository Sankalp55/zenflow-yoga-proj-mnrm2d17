...
export function NavbarSticky({ logo, navItems, ctaLabel, ctaHref, logoUrl }: NavbarStickyProps) {
  const [mobileOpen, setMobileOpen] = useState(false);

  return (
    <header className="sticky top-0 z-50 w-full border-b bg-background/80 backdrop-blur-md">
      <div className="container mx-auto flex h-16 max-w-7xl items-center justify-between px-4">
        <Link href="/" className="flex items-center gap-2">{logoUrl ? <img src={logoUrl} alt={logo || ""} className="h-8 w-auto" /> : <span className="text-xl font-bold text-foreground">{logo}</span>}</Link>
